with ts as (
    ...
    where barn.klassekode in ('TSLMASISP2-OP','TSLMASISP4-OP') --Læringsmidler
...
lm_mottaker_data as (
  SELECT
    ...
    ts.klassekode,
    SUM(CASE WHEN to_char(ts.DATO_UTBET_FOM,'YYYYMM')= {{ var ('periode')}} THEN ts.BELOP ELSE 0 END)  TSLM,
    SUM(CASE WHEN to_char(ts.DATO_UTBET_FOM,'YYYYMM')< {{ var ('periode')}} THEN ts.BELOP ELSE 0 END) TSLM_ETTERBETALT,
